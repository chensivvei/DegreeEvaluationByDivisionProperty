...
#include <bitset>
using namespace std;
typedef bitset<288> uint288;    /* 228 = 93 + 84 + 111 */
typedef vector<uint288> Monomial;
typedef struct {
	int monomial_num;
	int degree;
	Monomial monomial;
...
void remove_or_add_term(Poly &poly, uint288 monomial)
{
	vector<bitset<288>>::iterator it = find(poly.monomial.begin(), poly.monomial.end(), monomial);
...
/* calculate the algebraic degree of a given polynomial */
...
void state_update(Poly* state)
{
	...
	t[2] = poly_add(state[242], state[287]);
...
Monomial remove_no_max_term(Monomial monomial, uint288 low_deg_term)
{
...
int get_max_poly(Poly z, int round)
{
	...
	{
		uint288 tmp_term = max_monomial[i] & tmp;
