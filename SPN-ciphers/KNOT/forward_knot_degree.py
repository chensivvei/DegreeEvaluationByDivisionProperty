...
class KNOT(object):
	...
	def nonlinear_layer(self,input,round):
	...
	def constraints(self):
		...
		# self.f.write("obj >= "+str(self.deg)+"\n")
		for i in range(self.round-1):
			state = self.nonlinear_layer(state, i)
			state = self.word_rotate(state)
		state = self.nonlinear_layer(state, self.round - 1)
...
def degree_evaluate(t, degree, round, block_size):
	global bound
	file_name = "forward-knot%d.lp"%block_size
	bound = degree
	input_dp_wt = degree 
	output_dp_wt = 0
	while output_dp_wt <= degree and input_dp_wt < block_size:
		input_dp_wt += 1
		f = open(file_name,"w")
		knot = KNOT(t, input_dp_wt, file_name, f, int(block_size/4))
		knot.creat_model()
		f.close()
...
def main():
	...
	t = int(input("t: "))
	f = open("forward%d-result.txt"%block_size,"a")
	f.write("### t = %d ###\n"%t)
